...
@NamedQueries( {
	...
	@NamedQuery(name = "BDO.findBdoCabal", query = "select new com.oji.finonline.helper.BdoCriteria(c.sol_id,c.schm_code,c.schm_type,c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " group by c.sol_id,c.schm_code,c.acct_crncy_code,c.schm_type "
			+ " order by c.sol_id,c.schm_code,c.acct_crncy_code"),

	@NamedQuery(name = "BDO.findBdoCabalCredit", query = "select new com.oji.finonline.helper.BdoCriteria(c.sol_id,c.schm_code,c.schm_type,c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " and c.clr_bal_amt >= 0 and c.schm_type != 'LAA' "
			+ " group by c.sol_id,c.schm_code,c.acct_crncy_code,c.schm_type "
			+ " order by c.sol_id,c.schm_code,c.acct_crncy_code"),
					
	@NamedQuery(name = "BDO.findBdoCabalDebit", query = "select new com.oji.finonline.helper.BdoCriteria(c.sol_id,c.schm_code,c.schm_type,c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " and c.clr_bal_amt < 0 or ( c.schm_type = 'LAA' and c.acct_mgr_user_id = :uprId) "
			+ " group by c.sol_id,c.schm_code,c.acct_crncy_code,c.schm_type "
			+ " order by c.sol_id,c.schm_code,c.acct_crncy_code"),
					
	@NamedQuery(name = "BDO.findBdoCabalByCurrency", query = "select new com.oji.finonline.helper.BdoCriteria(c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " group by c.acct_crncy_code "
			+ " order by c.acct_crncy_code"),
	
	@NamedQuery(name = "BDO.findBdoCabalByCurrencyCredit", query = "select new com.oji.finonline.helper.BdoCriteria(c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " and c.clr_bal_amt >= 0  and c.schm_type != 'LAA' "
			+ " group by c.acct_crncy_code "
			+ " order by c.acct_crncy_code"),
		
	@NamedQuery(name = "BDO.findBdoCabalByCurrencyDebit", query = "select new com.oji.finonline.helper.BdoCriteria(c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " and c.clr_bal_amt < 0 or ( c.schm_type = 'LAA' and c.acct_mgr_user_id = :uprId) "
			+ " group by c.acct_crncy_code "
			+ " order by c.acct_crncy_code"),
					
	@NamedQuery(name = "BDO.findBdoCabalBySolId", query = "select new com.oji.finonline.helper.BdoCriteria(count(c),c.sol_id,sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " group by c.sol_id "
			+ " order by c.sol_id"),
					
		
	@NamedQuery(name = "BDO.findBdoCabalByScheme", query = "select new com.oji.finonline.helper.BdoCriteria(c.schm_code,c.schm_type,c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " group by c.schm_code,c.schm_type,c.acct_crncy_code "
			+ " order by c.schm_code,c.schm_type,c.acct_crncy_code"),
			
	@NamedQuery(name = "BDO.findBdoCabalBySchemeCredit", query = "select new com.oji.finonline.helper.BdoCriteria(c.schm_code,c.schm_type,c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " and c.clr_bal_amt >= 0 and  c.schm_type != 'LAA' "
			+ " group by c.schm_code,c.schm_type,c.acct_crncy_code "
			+ " order by c.schm_code,c.schm_type,c.acct_crncy_code"),
				
	@NamedQuery(name = "BDO.findBdoCabalBySchemeDebit", query = "select new com.oji.finonline.helper.BdoCriteria(c.schm_code,c.schm_type,c.acct_crncy_code,count(c),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " and c.clr_bal_amt < 0 or ( c.schm_type = 'LAA' and c.acct_mgr_user_id = :uprId) "
			+ " group by c.schm_code,c.schm_type,c.acct_crncy_code "
			+ " order by c.schm_code,c.schm_type,c.acct_crncy_code"),
					
	@NamedQuery(name = "BDO.findCustomerCabal", query = "select new com.oji.finonline.helper.BdoCriteria(c.sol_id,c.schm_code,c.schm_type,c.acct_crncy_code,count(c.schm_code),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.cif_id = :cifId and c.del_flg = 'N' "
			+ " group by c.sol_id,c.schm_code,c.acct_crncy_code,c.schm_type "
			+ " order by c.sol_id,c.schm_code,c.acct_crncy_code"),
		
	@NamedQuery(name = "BDO.findCustomerCabalByBdo", query = "select new com.oji.finonline.helper.BdoCriteria(c.sol_id,c.schm_code,c.schm_type,c.acct_crncy_code,count(c.schm_code),sum(case when c.schm_type = 'LAA' then  (c.clr_bal_amt * -1) else c.clr_bal_amt end)) "
			+ " from GAM c  where  c.cif_id = :cifId and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " group by c.sol_id,c.schm_code,c.acct_crncy_code,c.schm_type "
			+ " order by c.sol_id,c.schm_code,c.acct_crncy_code"),
					
					
	@NamedQuery(name = "BDO.findAccountByCriteria1", query = "select c from GAM c  "
			+ " where  c.sol_id = :solId and c.schm_code = :schmCode and c.acct_crncy_code = :currency "
			+ " and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " order by c.foracid "),
			
	@NamedQuery(name = "BDO.findAccountByCriteria1SolId", query = "select c from GAM c  "
			+ " where  c.sol_id = :solId  "
			+ " and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " order by c.schm_code,c.foracid "),			
			
	@NamedQuery(name = "BDO.findAccountByCriteria1Scheme", query = "select c from GAM c  "
			+ " where  c.schm_code = :schmCode "
			+ " and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " order by c.sol_id,c.schm_code,c.foracid "),
					
					
	@NamedQuery(name = "BDO.findAccountByCriteria1Currency", query = "select c from GAM c  "
			+ " where  c.acct_crncy_code = :currency "
			+ " and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " order by c.foracid "),
		
	@NamedQuery(name = "BDO.findAccountByOpenedDateCurrencyManager", query = "select c from GAM c  "
			+ " where  c.acct_crncy_code = :currency "
			+ " and c.acct_opn_date = :openedDate "
			+ " and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " order by c.foracid "),
			
	@NamedQuery(name = "BDO.findAccountByLienCurrencyManager", query = "select c from GAM c  "
			+ " where  c.acct_crncy_code = :currency "
			+ " and c.lien_amt != 0 "
			+ " and c.acct_mgr_user_id = :uprId and c.del_flg = 'N' "
			+ " order by c.foracid "),
					
	@NamedQuery(name = "BDO.findAccountByCriteria2", query = "select c from GAM c  "
			+ " where  c.sol_id = :solId and c.schm_code = :schmCode and c.acct_crncy_code = :currency "
			+ " and c.cif_id = :cifId and c.del_flg = 'N' "
			+ " order by c.foracid "),
	@NamedQuery(name = "BDO.findAccountByCustId", query = "select c from GAM c  "
			+ " where  c.cif_id = :cifId and c.acct_mgr_user_id = :uprId "
			+ " and c.del_flg = 'N' "
			+ " order by c.foracid ")
})
