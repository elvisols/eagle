...
public class CustomerService {
	...
	public Response getAllUsers(@HeaderParam("token") String token, @QueryParam("start") Integer start,
		...
		resp.setPayload(cmgDao.getAllCustomers(user,start,size));
		resp.setStatusMessages(al);
		resp.setStatus(200);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/search")
	public Response search(@HeaderParam("token") String token, CustomerSearchDTO query,
		...
		resp.setPayload(cmgDao.search(user,start, size, query));
		resp.setStatusMessages(al);
		resp.setStatus(200);
