 ...
 */
@Entity
@Table(name="finacle.dtd")
@NamedQueries( {
	@NamedQuery(name = "DTD.findByCustId", query = "select c from GAM c  where  c.cust_id = :cust_id"),
	@NamedQuery(name = "DTD.findCotByAccount", 
		query = "select c from HTD c  where  c.id.tran_date = :tranDate and c.acid = :acid "
				+ " and c.tran_sub_type = 'CI' and c.del_flg = 'N' "
				+ " order by c.id.tran_id "),
	@NamedQuery(name = "DTD.findTxnByAccount",  //c.id.tran_date = :tranDate and
		query = "select c from DTD c  where   c.acid = :acid "
		+ " and c.del_flg = 'N' order by c.id.tran_id "),
	@NamedQuery(name = "DTD.findTxnByAccountByTranId", 
	query = "select c from DTD c  where  c.id.tran_date = :tranDate and c.acid = :acid "
		+ " and c.del_flg = 'N'  and c.id.tran_id = :tranId order by c.id.tran_id "),
	@NamedQuery(name = "DTD.findTxnByTranDateAndTranId", 
	query = "select c from DTD c  where  c.id.tran_date = :tranDate  "
		+ " and c.del_flg = 'N'  and c.id.tran_id = :tranId order by c.id.tran_id ")		
})
public class DTD implements Serializable{
	...
	@Column(length=1)
	private String  part_tran_type      ; //  char(1 byte),
